...
registerMooseObject("RhocpApp", DDCPStressUpdate2);

InputParameters
DDCPStressUpdate2::validParams()
{
...
DDCPStressUpdate2::DDCPStressUpdate2(const InputParameters & parameters) :
...
DDCPStressUpdate2::~DDCPStressUpdate2()
{
}

void DDCPStressUpdate2::initQpStatefulProperties()
{
...
void DDCPStressUpdate2::computeQpStress()
{
...
//       }
    ...
    for(unsigned int ia = 0; ia < _num_slip_sys; ia++) {
      ...
      temp2[ia][ia] = temp2[ia][ia]
                                   + (k_M/(b_mag*d_disl[ia]) 
                                   -  k_I/(b_mag*d_disl4[ia])
                                   - (k_ann*R_c*rho_m[ia]/b_mag)
                                   +  k_D*rho_i[ia])*(sgn(gamma_dot[ia])*dt_incr);
   ...
   std::vector<Real> d_disl2(_num_slip_sys); // Mean Free Path
   std::vector<Real> d_disl3(_num_slip_sys);   
  ...
  if(_climbmodel){
    ...
    for (unsigned int ia = 0; ia <_num_slip_sys; ia++) {
      for (unsigned int ib = 0; ib < _num_slip_sys; ib++ ) {
        drhomdgc[ia][ib] = 0.0;
        for(unsigned int ic = 0; ic < _num_slip_sys; ic++) {
          drhomdgc[ia][ib] = drhomdgc[ia][ib] + temp11_inv[ia][ic]*temp12[ic][ib];
        }
      }
    }
    ...
    for (unsigned int ia = 0; ia < _num_slip_sys; ia++){
      for(unsigned int ib = 0; ib < _num_slip_sys; ib++){
        Real t1 = pow(rho_m[ia], -0.5)/(2*b_mag);
        drhoidgc[ia][ib] = t1*drhomdgc[ia][ib]*abs(gamma_dot_c[ia])*dt_incr;
      }
      drhoidgc[ia][ia] = drhoidgc[ia][ia] + (1/b_mag/d_disl2[ia] - k_D2*rho_i[ia])*sgn(gamma_dot_c[ia])*dt_incr;
    }
    ...
    if(_climbmodel){
      ...
      for(unsigned int ia = _num_slip_sys; ia< (totalsys); ia++){
        ...
        for(unsigned int ib= _num_slip_sys; ib< (totalsys); ib++){
          ...
          lg = d_disl2[ik];
          array3[ia][ib] = - (   (drhomdgc[ij][ik]*b_mag*vel_climb[ij]*lg/lc)
                              + (rho_m[ij]*b_mag*dvcdgc[ij][ik]*lg/lc) 
                              + (rho_m[ij]*b_mag*vel_climb[ij]*dldgc[ij][ik]/lc) );
        }
        array3[ia][ia] = array3[ia][ia] + 1;
      }
    }
...
// NR_residual()
void DDCPStressUpdate2::NR_residual (
  ...
  std::vector<Real> d_disl2(num_slip_sys);
  std::vector<Real> d_disl3(num_slip_sys);
  for(unsigned int ia = 0; ia < num_slip_sys; ia++) {
    ...
    for(unsigned int ib = 0; ib < num_slip_sys; ib++) {
      ...
      sum3 = sum3 + H[ia][ib]*(rho_i0[ib]);
    }
    d_disl[ia] = 1.0e0/sqrt(sum1);
    d_disl2[ia] = pow((sqrt(sum2)), -1);
    d_disl3[ia] = 1.0e0/sqrt(sum3);
  }
  ...
  for(unsigned int ia = 0; ia < num_slip_sys; ia++) {
    ...
    drhomdt[ia] = ((k_M/b_mag/d_disl[ia]                          // Dislocation Multiplication Component
                 ...
                 - (k_I*sqrt(sum3))/b_mag)*abs(gdt[ia])           // Trapped Dislocations or Junction Formation Rate
                 + k_D*rho_i0[ia]*abs(gdt[ia])                    // Junction Dissolution Rate
                 - abs(gdc[ia])/(b_mag*d_disl2[ia]))              // Climb Component Annhilation
    ...
    drhoidt[ia] = ((k_I*sqrt(sum3))/b_mag                       // Trapped Dislocation - Glide Component or Junction Formation Rate
                  - k_D*rho_i0[ia])*abs(gdt[ia])                // Dynamic Recovery of Dislocations or Junction Dissolution Rate
                  + abs(gdc[ia])/(b_mag*d_disl2[ia])            // Climb Component Addition
                  - k_D2 * rho_i0[ia]* abs(gdc[ia]);            // Relaxation of Immobile Dislocations due to Climb


    if (_irad_defects) {

    dNloopdt[ia] = abs(gdc[ia])/(b_mag*d_disl2[ia]*pi*dloop0[ia]) // Increase in Number Density Dislocation Loops Due to Climb
...
void DDCPStressUpdate2::readPropsFile() {
...
void DDCPStressUpdate2::assignProperties(){
...
//   if (_grain_size >= 1.e10) {
  ...
  mfp_precp = _properties[_qp][47]; // Mean Free Path of Precipitates
  ...
  tauSC =  _properties[_qp][52]; // Hardening of Solute Clusters
...
void DDCPStressUpdate2::normalize_vector(Real *x, Real *y, Real *z) {
...
void DDCPStressUpdate2::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void DDCPStressUpdate2::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
...
void DDCPStressUpdate2::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void DDCPStressUpdate2::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void DDCPStressUpdate2::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
void DDCPStressUpdate2::aa_dot_bb(Real a[3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real DDCPStressUpdate2::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void DDCPStressUpdate2::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real DDCPStressUpdate2::power(Real x, Real y) {
...
Real DDCPStressUpdate2::sgn(Real x) {
...
Real DDCPStressUpdate2::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
...
int DDCPStressUpdate2::numrows(std::string filename){
    int numlines{0};
...
int DDCPStressUpdate2::numcols(std::string filename){
    int columncount{0};
    std::ifstream file(filename);
    if (!file.is_open()){
...
void DDCPStressUpdate2::readfile(std::string filename){
...
void DDCPStressUpdate2::calc_crss(
      ...
      if (_crsslaw == 1) // Default CRSS LAW taking same strength of Mobile and Immobile Dislocations
      {
          for (unsigned int ia = 0; ia < _num_slip_sys; ia++) {
          Real sum1 = 0.0; 
          ...
          if(_irad_defects)
          { s_a2 = alpha_loop* G * b_mag*sqrt(Nloop[ia]*dloop[ia]);
            s_a3 = tauSC;
          }
      ...
      else if (_crsslaw == 2) // CRSS Law Taking Different Strength of Mobile and Immobile Dislocations
      {
        // In this CRSS Law Strength Coefficient of Immobile Dislocations = 1.0*k_rho
        //              and Strength Coefficient of Mobile   Dislocations = 0.1*K_rho
        Real k_rhom = 0.1*k_rho,k_rhoi = k_rho;
          for (unsigned int ia = 0; ia < _num_slip_sys; ia++) {
...
void DDCPStressUpdate2::calc_dsadgb(
  unsigned int num_slip_sys,
  std::vector<Real> &rho_m,
  std::vector<Real> &rho_i,
  std::vector<Real> &Nloop,
  ...
  if(_crsslaw == 1){
    ...
    {
    Real k_rhom = 0.1*k_rho, k_rhoi = 1.0*k_rho;
    for(unsigned int ia = 0; ia < num_slip_sys; ia++) {
      for(unsigned int ib = 0; ib < num_slip_sys; ib++){
      Real sum1 = 0.0, sum2 = 0.0;
      for(unsigned int ib1 = 0; ib1 < num_slip_sys; ib1++) {
      ...
      void DDCPStressUpdate2::calc_sliprate(
        unsigned int num_slip_sys,
      ...
      void DDCPStressUpdate2::calc_climbrate(
        unsigned int num_slip_sys,
        Real Di,
        Real Dv,
        Real vacdensity_th,
        ...
        for (unsigned int k = 0; k < _num_slip_sys ; k++) {
          ...
          lg = d_disl2[k]; //1/sqrt(rho_m[k]);
          gamma_dot_c[k] = rho_m[k] * b_mag * (lg / lc) * vel_climb[k];
