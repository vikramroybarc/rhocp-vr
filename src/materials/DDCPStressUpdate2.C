...
void DDCPStressUpdate2::NR_residual (
  ...
  for(unsigned int ia = 0; ia < num_slip_sys; ia++) { 
    ...
    drhoidt[ia] =  (k_I/b_mag/d_disl[ia]*abs(gdt[ia]))         // Trapped Dislocation - Glide Component or Junction Formation Rate
                  - k_r*rho_i0[ia]*abs(gdt[ia])              //  Junction Dissolution Rate
                  - k_D*rho_i0[ia]*abs(gdt[ia]);              // Dynamic Relaxtion of Immobile Dislocations
...
//   if (_grain_size >= 1.e10) {
  ...
  k_D_0 = _properties[_qp][34]; // Immobile Dislocation Evolution Dynamic Recovery Constant
  n0 = _properties[_qp][35]; // Dynamic Recovery strain rate exponent
  edot0 = _properties[_qp][36]; // Referance Strain Rate for Dynamic Recovery
...
void DDCPStressUpdate2::calc_dsadgb(
 ...
 Real DDCPStressUpdate2::calc_w(
  ...
  void DDCPStressUpdate2::calc_taup(
  unsigned int num_slip_sys,
  std::vector<Real> &s_t,
  std::vector<Real> &gamma_dot,
  std::vector<Real> &tau_p,
