...
// NR_residual()
void DDCPStressUpdate::NR_residual (
  ...
  for(unsigned int ia = 0; ia < num_slip_sys; ia++) {
    ...
    drhomdt[ia] = ((k_M/b_mag/d_disl[ia]                          // Dislocation Multiplication Component
                 ...
                 - (k_I*sqrt(sum3))/b_mag)*abs(gdt[ia])           // Trapped Dislocations
    ...
    drhoidt[ia] = ((k_I*sqrt(sum3))/b_mag                       // Trapped Dislocation - Glide Component
...
int DDCPStressUpdate::numrows(std::string filename){
    int numlines{0};
...
int DDCPStressUpdate::numcols(std::string filename){
    int columncount{0};
    std::ifstream file(filename);
    if (!file.is_open()){
...
void DDCPStressUpdate::readfile(std::string filename){
...
void DDCPStressUpdate::calc_crss(
      ...
      void DDCPStressUpdate::calc_sliprate(
        unsigned int num_slip_sys,
      ...
      void DDCPStressUpdate::calc_climbrate(
        unsigned int num_slip_sys,
        Real Di,
        Real Dv,
        Real vacdensity_th,
