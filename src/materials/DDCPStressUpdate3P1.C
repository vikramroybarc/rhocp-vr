// Date 03-07-2024
// Vikram Roy, BARC Mumbai
// Modification of Default File so as to incorpurate climb strains

#include "DDCPStressUpdate3P1.h"
...
registerMooseObject("RhocpApp", DDCPStressUpdate3P1);

InputParameters
DDCPStressUpdate3P1::validParams()
{
  ...
  params.addParam<bool>("Saturated_PointDefects", true, "Whether Consider Saturated Point Defects in the model");
...
DDCPStressUpdate3P1::DDCPStressUpdate3P1(const InputParameters & parameters) :
    ...
    _saturated_point_defects(getParam<bool>("Saturated_PointDefects")),
...
DDCPStressUpdate3P1::~DDCPStressUpdate3P1()
{
}

void DDCPStressUpdate3P1::initQpStatefulProperties()
{
...
void DDCPStressUpdate3P1::computeQpStress()
{
  ...
  // Variables for Point Defects when their Saturation is not considered
  Real ci, ci0; //interstitial density at the start and end of step
  Real cv, cv0; //vacancy density at the start and end of step
  ...
  if (_t_step == 1) {
    ...
    if(_climbmodel && !_saturated_point_defects)
     {
          if (K0 > 0){
          ci0 = 1e12; // Initial Values of Point Defects Units -> mm2/s
          cv0 = 1e12; // Initial Values of Point Defects Units -> mm2/s
          }
  ...
  if(_t_step > 1) {
    ...
    // Read Point Defect Densities  SDV50 + _num_slip_sys*7 + 4 &   SDV50 + _num_slip_sys*7 + 5
    if(_climbmodel && !_saturated_point_defects)
    {
      n = 50 + 7*_num_slip_sys + 4 -1;
      ci0 = _state_var[_qp][n];

      n = 50 + 7*_num_slip_sys + 5 -1;
      cv0 = _state_var[_qp][n];
  ...
  if(_climbmodel){
    ...
    // Pass the Unsaturated Variables to Climb Model
    if(!_saturated_point_defects)
    {
      interdensity = ci0;
      vacdensity = cv0;
    }
    ...
    // Save the saturated density in the variables
    if(_saturated_point_defects){
      ci = interdensity;
      ci0 = interdensity;
      cv = vacdensity;
  ...
  do {
    ...
    // Reassign newly calculated ci and cv to interdensity and vacdensity
    interdensity = ci;
    vacdensity = cv;
...
//       }
  ...
  // reassign ci and cv to interdensity and vacdensity
  interdensity = ci;
  vacdensity = cv;
  ...
  // Store Point Defects SDV50 + _num_slip_sys*7 + 5  
  if(_climbmodel) {
    n = n+1;
    _state_var[_qp][n] = ci;
...
// NR_residual()
void DDCPStressUpdate3P1::NR_residual (
  ...
  Real &ci0,                            // Density of Interstitials at the begining of step
  Real &ci,                             // Density of Interstitials at the end of Step
  Real &cv0,                            // Density of Vacancies at the begining of step
  Real &cv,                             // Density of Vacancies at the end of steps 
  ...
  // Save the results into local vars interdensity and vacdensity
  Real interdensity, vacdensity;
  // (Vik) Variables for Calculation
  ...
  if(!_saturated_point_defects){
     zi[ia] = zi0 + (Zsi/G) * climbstress[ia];
     zv[ia] = zv0;
     Kis[ia] = abs(zi[ia]) * Di;
     Kvs[ia] = abs(zv[ia]) * Dv;
     vacdensity_0[ia] = vacdensity_th * exp((climbstress[ia] * atomvol * Ecorr)/(B_k*_temp[_qp])); 
     if (vacdensity_0[ia] > max_vac_thermal){
       max_vac_thermal = vacdensity_0[ia];
     }
     ci_absorbed = ci_absorbed + Kis[ia]*(rho_i0[ia] +rho_m0[ia])*ci0;
     cv_absorbed = cv_absorbed + Kvs[ia]*(rho_i0[ia] +rho_m0[ia])*(cv0 - vacdensity_0[ia]);
  }
  ...
  // New Density of Point Defects
  Real cidot, cvdot;
  if(!_saturated_point_defects){
    cidot = K0 - ci_absorbed;
    cvdot = K0 - cv_absorbed;
    ci = ci0 + cidot*dt;
    cv = cv0 + cvdot*dt;
  ...
  //   if (cv < vacdensity_0[ia]){
  //     cv = vacdensity_0[ia];
  //   }
  // }


  // Change the value assignment for calculations
  interdensity = ci;
  vacdensity = cv;
  ...
  for(unsigned int ia=0; ia< num_slip_sys; ia++){
    ...
    dNloopdt[ia] = k_c*rho_m0[ia]*abs(gdc[ia])/(pi*dloop0[ia]) // Increase in Number Density Dislocation Loops Due to Climb
      - (1/(2*b_mag))*sqrt(Nloop0[ia]*dloop0[ia])*sqrt(rho_m0[ia])*abs(gdt[ia]); // Decrease in Number Density of Dislocation Loops due to Annhilation
...
void DDCPStressUpdate3P1::readPropsFile() {
...
void DDCPStressUpdate3P1::assignProperties(){
...
void DDCPStressUpdate3P1::normalize_vector(Real *x, Real *y, Real *z) {
...
void DDCPStressUpdate3P1::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void DDCPStressUpdate3P1::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
...
void DDCPStressUpdate3P1::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void DDCPStressUpdate3P1::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void DDCPStressUpdate3P1::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
void DDCPStressUpdate3P1::aa_dot_bb(Real a[3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real DDCPStressUpdate3P1::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void DDCPStressUpdate3P1::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real DDCPStressUpdate3P1::power(Real x, Real y) {
...
Real DDCPStressUpdate3P1::sgn(Real x) {
...
Real DDCPStressUpdate3P1::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
...
int DDCPStressUpdate3P1::numrows(std::string filename){
    int numlines{0};
...
int DDCPStressUpdate3P1::numcols(std::string filename){
    int columncount{0};
    std::ifstream file(filename);
    if (!file.is_open()){
...
void DDCPStressUpdate3P1::readfile(std::string filename){
...
void DDCPStressUpdate3P1::calc_crss(
      ...
      void DDCPStressUpdate3P1::calc_sliprate(
        unsigned int num_slip_sys,
      ...
      void DDCPStressUpdate3P1::calc_climbrate(
        unsigned int num_slip_sys,
        Real Di,
        Real Dv,
        Real vacdensity_th,
     ...
     void DDCPStressUpdate3P1::calc_mfp(
      unsigned int num_slip_sys,
      std::vector<Real> &rho_m,
      std::vector<Real> &rho_i,
      std::vector<Real> &Nloop,
