...
// Modification to introduce Phase Field Coupling with raccoon

#include "DDCPStressUpdate4.h"
...
registerMooseObject("RhocpApp", DDCPStressUpdate4);

InputParameters
DDCPStressUpdate4::validParams()
{
  ...
  params.addParam<MaterialPropertyName>("strain_energy_density", "psie", "Name of the strain energy density computed by this material model");
  params.addParam<MaterialPropertyName>("plastic_energy_density", "psip", "Name of the plastic energy density computed by this material model");
  params.addParam<MaterialPropertyName>("elastic_degradation_function", "ge", "The Elastic Energy degradation function");
  params.addParam<MaterialPropertyName>("plastic_degradation_function", "gp", "The Plastic Energy degradation function");
  return params;
}

DDCPStressUpdate4::DDCPStressUpdate4(const InputParameters & parameters) :
    ...
    // Strain Energy Density and its derivatives
    _psie_name("strain_energy_density", true),
    ...
    _psip_name("plastic_energy_density", true),
...
DDCPStressUpdate4::~DDCPStressUpdate4()
{
}

void DDCPStressUpdate4::initQpStatefulProperties()
{
...
void DDCPStressUpdate4::computeQpStress()
{
...
// NR_residual()
void DDCPStressUpdate4::NR_residual (
...
void DDCPStressUpdate4::readPropsFile() {
...
void DDCPStressUpdate4::assignProperties(){
...
void DDCPStressUpdate4::normalize_vector(Real *x, Real *y, Real *z) {
...
void DDCPStressUpdate4::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void DDCPStressUpdate4::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
...
void DDCPStressUpdate4::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void DDCPStressUpdate4::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void DDCPStressUpdate4::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real DDCPStressUpdate4::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void DDCPStressUpdate4::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real DDCPStressUpdate4::power(Real x, Real y) {
...
Real DDCPStressUpdate4::sgn(Real x) {
...
Real DDCPStressUpdate4::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
...
void DDCPStressUpdate4::calc_crss(
      ...
      void DDCPStressUpdate4::calc_sliprate(
        unsigned int num_slip_sys,
        ...
        for(unsigned int k = 0; k < _num_slip_sys; k++) {
          ...
          if(abs(tau_eff[k]) > s_a[k]) {
            ...
            tau_eff[k] = tau_eff[k]/MetaPhysicL::raw_value(_gp[_qp]);
      ...
      void DDCPStressUpdate4::calc_climbrate(
        unsigned int num_slip_sys,
        Real Di,
        Real Dv,
        Real vacdensity_th,
     ...
     void DDCPStressUpdate4::calc_mfp(
      unsigned int num_slip_sys,
      std::vector<Real> &rho_m,
      std::vector<Real> &rho_i,
      std::vector<Real> &Nloop,
    ...
    cauchyStress = MetaPhysicL::raw_value(_ge[_qp]) * cauchyStress_intact;
    ...
    if(converged){
      ...
      _psie[_qp] = _ge[_qp] *  _psie_active[_qp];
      _dpsie_dd[_qp] = _dge_dd[_qp] * _psie_active[_qp];
    }
  ...
  RankTwoTensor DDCPStressUpdate4::computeCauchyStressVolDevDecomposition(
    ...
    cauchyStress = MetaPhysicL::raw_value(_ge[_qp]) * sig_active + sig_inactive;
    ...
    if(converged){
      ...
      _psie[_qp] = _ge[_qp] * _psie_active[_qp] + psie_inactive;
      _dpsie_dd[_qp] = _dge_dd[_qp] * _psie_active[_qp];
    }
