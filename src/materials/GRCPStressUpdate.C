// Need to correct the formula of dsadgb

#include "GRCPStressUpdate.h"
...
registerMooseObject("RhocpApp", GRCPStressUpdate);

InputParameters
GRCPStressUpdate::validParams()
{
  InputParameters params = ComputeStressBase::validParams();
  params.addClassDescription("Stress calculation using the Crystal Plasticity material model for Graphite crystals");
...
GRCPStressUpdate::GRCPStressUpdate(const InputParameters & parameters) :
...
GRCPStressUpdate::~GRCPStressUpdate()
{
}

void GRCPStressUpdate::initQpStatefulProperties()
{
...
void GRCPStressUpdate::computeQpStress()
{
  ...
  std::vector<Real> sa_hard0(_num_slip_sys); // Hardening due to Plastic Deformation Previous Time Step
  std::vector<Real> sa_hard(_num_slip_sys); // Hardening due to Plastic Deformation
  ...
  if(_t_step > 1) {
   ...
   // // Read mobile dislocation density values SDV 50 + (_num_slip_sys - _num_twin_sys)
    ...
    //   n = n + 1;
      rho_m0[i] = 100.0 ; //_state_var[_qp][n];
    }

   // // Read immobile dislocation density values SDV 50 + (_num_slip_sys - _num_twin_sys)*2
    for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    //   n = n + 1;
      rho_i0[i] = 100.0;// _state_var[_qp][n];
    }

    // Read back stress values SDV 50  (_num_slip_sys - _num_twin_sys)*1
    for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
      n = n + 1;
      bstress0[i] = _state_var[_qp][n];
    }

    // Read Hardening due to Plasticity SDV 50 + (_num_slip_sys - _num_twin_sys)*2
    for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
      n = n + 1;
      sa_hard0[i] = _state_var[_qp][n];
    }

    // Read twin fractions SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys
    for (unsigned int i = 0; i < _num_twin_sys; i++) {
      n = n + 1;
      twin_fraction0[i] = _state_var[_qp][n];
    }

    // Read twin variant SDV 50 + (_num_slip_sys - _num_twin_sys)*2  + _num_twin_sys + 1
    n = n + 1;
    itvariant = _state_var[_qp][n];

    // Read gamma_twin_ref SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys + 2
    n = n + 1;
...
//   C0[2][2][2][2] = C11;
  ...
  for (unsigned int ia = 0; ia < (_num_slip_sys - _num_twin_sys); ia++) {
    ...
    // }
    s_a[ia] = tau0[isx(ia)] + hp_coeff[isx(ia)]/sqrt(grain_size[isx(ia)]) + sa_hard0[ia]; //+     k_rho[isx(ia)]*G*b_mag[isx(ia)]*sqrt(sum1) ;
  }
...
//       }
    ...
    std::vector<std::vector<Real>> temp11((_num_slip_sys - _num_twin_sys), std::vector<Real>((_num_slip_sys - _num_twin_sys)));
    std::vector<std::vector<Real>> temp12((_num_slip_sys - _num_twin_sys), std::vector<Real>((_num_slip_sys - _num_twin_sys))); 
       std::vector<std::vector<Real>> dhdg((_num_slip_sys), std::vector<Real>((_num_slip_sys)));   
    ...
    // Terms reqd in Calculation of dsadgb
    for(unsigned int ia = 0; ia < (_num_slip_sys ); ia++) {

      if(ia < 3){ //basal slip

      dhdg[ia][ia] = h0[isx(ia)]*sgn(gamma_dot_g[ia])*dt_incr;

      }
      else{
        Real x1 = s_a[ia]/ss[isx(ia)];
        Real x2 = h0[isx(ia)]*pow(1- x1,  m[isx(ia)]);
        dhdg[ia][ia] = x2*sgn(gamma_dot_g[ia])*dt_incr;
      }
    ...
    for(unsigned int ia = 0; ia < _num_slip_sys; ia++) {
      for(unsigned int ib = 0; ib < _num_slip_sys; ib++) {
        ...
        for(unsigned int ic = 0; ic < _num_slip_sys; ic++) {
          dsadgb[ia][ib] =dsadgb[ia][ib] + A[ia][ic]*dhdg[ic][ib];
        }
      }
    }    
      ...
      for (unsigned int k = 0; k < 3; k++) {
        for (unsigned int l = 0; l < 3; l++) {
          ...
          for (unsigned int n = 0; n < (_num_slip_sys - _num_twin_sys); n++) {
            ...
            (gamma_dot_g[n]*(p[isx(n)]*q[isx(n)]*(delF0/B_k/_temp[_qp])*
            power(1 - power((abs(tau_eff[n]) - s_a[n])/s_t[n], p[isx(n)]), q[isx(n)] - 1)*power((abs(tau_eff[n])
...
//   }
  ...
  // Read backstress values SDV 50 + (_num_slip_sys - _num_twin_sys)*1
  for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    n = n + 1;
    _state_var[_qp][n] = bstress[i];
  }

  // Store Hardening due to Plasticity sa_hard SDV 50 + (_num_slip_sys - _num_twin_sys)*2
  for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    n = n + 1;
    _state_var[_qp][n] = sa_hard[i];
  }  
  ...
  // Relative deformation system activitiy SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys + 6
  Real gtot = 0.0e0;
  ...
  // store slip rate of each of the slip system SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys + 6 + (_num_slip_sys - _num_twin_sys)
  for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    n = n + 1;
    _state_var[_qp][n] = gamma_dot[i];
  }

  // store tau of each of the slip system SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys + 6 + (_num_slip_sys - _num_twin_sys)*2
  for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    n = n + 1;
    _state_var[_qp][n] = tau_eff[i];
  }

  // store sa of each of the slip system SDV 50 + (_num_slip_sys - _num_twin_sys)*2 + _num_twin_sys + 6 + (_num_slip_sys - _num_twin_sys)*3
  for (unsigned int i = 0; i < (_num_slip_sys - _num_twin_sys); i++) {
    n = n + 1;
    _state_var[_qp][n] = s_a[i];
  }         
...
// NR_residual()
void GRCPStressUpdate::NR_residual (
  ...
  std::vector<Real> &s_a, 
  std::vector<Real> &s_t,
  std::vector<Real> &sa_hard0,
  std::vector<Real> &sa_hard, 
  ...
  std::vector<Real> hb(_num_slip_sys);
  for (unsigned int ia = 0; ia < (_num_slip_sys) ; ia++){

    if(ia < 3) // basal slip
    {
      hb[ia] = h0[isx(ia)]*abs(gdt[ia])*dt;
    }
    else
    {
      Real x1 = s_a[ia]/ss[isx(ia)];
      Real x2 = pow(1 -x1, m[isx(ia)]);
      hb[ia] = h0[isx(ia)]*x2*abs(gdt[ia])*dt;
    }
  ...
  for (unsigned int ia = 0; ia < (_num_slip_sys) ; ia++){
    Real sum1 = 0;
    for (unsigned int ib = 0; ib < (_num_slip_sys) ; ib++){
      sum1 = sum1 + A[ia][ib]*hb[ib];
    }
    sa_hard[ia] = sa_hard0[ia] + sum1;
  }
  ...
  for(unsigned int ia = 0; ia < (num_slip_sys - num_twin_sys); ia++) {
    ...
    s_a[ia] = tau0[isx(ia)] + hp_coeff[isx(ia)]/sqrt(grain_size[isx(ia)]) + sa_hard[ia]; // +k_rho[isx(ia)]*G*b_mag[isx(ia)]*sqrt(sum1) + ;
  }
...
void GRCPStressUpdate::readPropsFile() {
...
void GRCPStressUpdate::assignProperties(){
  ...
  for (unsigned int i=0; i<3; i++){
  b_mag[i] = _properties[_qp][12 + 14*i + 1];  // Burgers vector magnitude

  // Flow parameters
  gammadot0g[i] = _properties[_qp][12 + 14*i + 2];  // reference strain rate
  enthalpy_const[i] = _properties[_qp][12 + 14*i + 3];  // Multiplication constant for enthalpy term
  p[i] = _properties[_qp][12 + 14*i + 4];  // Shape parameter (dislocation glide)
  q[i] = _properties[_qp][12 + 14*i + 5];  // Shape parameter (dislocation glide)

  // Hardening parameters
  tau0[i] = _properties[_qp][12 + 14*i + 6];  // Athermal slip resistance constant
  hp_coeff[i] = _properties[_qp][12 + 14*i + 7];  // Hall-Petch coefficient <TBA>
  //   if (_grain_size >= 1.e10) {
  if (! _GrainAreaSize){
    grain_size[i] = _properties[_qp][12 + 14*i + 8];  // Grain size (for Hall-Petch term) <TBA>
  }
  else {
    // if the GrainAreaSize object exists, then grain_size is taken directly from the mesh
    grain_size[i] = _grain_size;
  }
  frictional_stress[i] = _properties[_qp][12 + 14*i + 9]; // Lattice frictional resistance
  p0[i] = 1.0; //_properties[_qp][12 + 25*i + 10];  // Dislcoation barrier strength
  k_rho[i] = 0.0; // _properties[_qp][12 + 25*i + 11];  // Taylor hardening parameter
  k_I[i] = 0.0; // _properties[_qp][12 + 25*i + 12];  // Mean free path constant (dislocation network)

  Alatent[i] = 0.0; // _properties[_qp][12 + 25*i + 13];  // Latent hardening coefficient

  // Dislocation evolution parameters
  rho_m_zero[i] = 1e2; // _properties[_qp][12 + 25*i + 14];  // Initial mobile dislocation density
  rho_i_zero[i] = 1e2; //_properties[_qp][12 + 25*i + 15];  // Initial immobile dislocation density
  d_disl_zero[i] = 1e-6; // _properties[_qp][12 + 25*i + 16];  // Initial dislocation line length
  k_M[i] = 0.0; // _properties[_qp][12 + 25*i + 17]; // Dislocation line generation constant
  R_c[i] = 1e-8; // _properties[_qp][12 + 25*i + 18];  // Critical capture radius for dislocation annihilation
  k_ann[i] = 0.0; // _properties[_qp][12 + 25*i + 19];  // Dislocation evolution annihilation constant
  k_D[i] = 0.0; //_properties[_qp][12 + 25*i + 20];  // Immobile dislocation evolution dynamic recovery constant
  k_bs1[i] = _properties[_qp][12 + 14*i + 10];  // Backstress evolution constant 1
  k_bs2[i] = _properties[_qp][12 + 14*i + 11];  // Backstress evolution constant 2
  h0[i] = _properties[_qp][12 + 14*i + 12];  // Hardening Rate
  ss[i] = _properties[_qp][12 + 14*i + 13];  // Saturation Hardening Stress
  m[i] = _properties[_qp][12 + 14*i + 14];  // Rate Hardening Exponent  

  }

  // Parameters for twin systems
  tau_twin = _properties[_qp][55]; // threshold stress for twin
  drag_twin0 = _properties[_qp][56]; // drag stress for twin
  gd0twin = _properties[_qp][57]; // reference shear rate for twin
  exp_twin = _properties[_qp][58]; // rate exponent for twin
  gamma_twin = _properties[_qp][59]; // characteristic shear strain for twin
  twin_frac_reorient = _properties[_qp][60]; // twin fraction at which reorientation occurs
  twin_hard = _properties[_qp][61]; // twin hardening parameter (in lieu of reorientation)
  twin_hard_exp = _properties[_qp][62]; // twin hardening exponent (in lieu of reorientaion)
...
unsigned int GRCPStressUpdate::isx(unsigned int islip) {
...
void GRCPStressUpdate::normalize_vector(Real *x, Real *y, Real *z) {
...
void GRCPStressUpdate::rotate_4th(Real a[3][3], Real b[3][3][3][3], Real (&c)[3][3][3][3]) {
  Real d[3][3][3][3];
...
void GRCPStressUpdate::forth_to_Voigt(Real a[3][3][3][3], Real (&b)[6][6]) {
  for(unsigned int i = 1; i <= 3; i++) {
...
void GRCPStressUpdate::Voigt_to_forth(Real b[6][6], Real (&a)[3][3][3][3]) {
  for(unsigned int i = 1; i <= 3; i++) {
    for(unsigned int j = 1; j <= 3; j++) {
      unsigned int ia = i;
      if(i != j) {
...
void GRCPStressUpdate::aaaa_dot_dot_bbbb(Real a[3][3][3][3], Real b[3][3][3][3], Real (&product)[3][3][3][3]) {
...
void GRCPStressUpdate::aaaa_dot_dot_bb(Real a[3][3][3][3], Real b[3][3], Real (&product)[3][3]) {
...
void GRCPStressUpdate::aa_dot_bb(Real a[3][3], Real b[3][3], Real (&product)[3][3]) {
...
Real GRCPStressUpdate::aa_dot_dot_bb(Real a[3][3], Real b[3][3]) {
...
void GRCPStressUpdate::bunge_angles(Real (&array1)[3][3], Real (&psi0)[3]) {
...
Real GRCPStressUpdate::power(Real x, Real y) {
...
Real GRCPStressUpdate::sgn(Real x) {
...
Real GRCPStressUpdate::max_val(Real a, Real b)
{
  if (a>b)
  {
    return a;
...
int GRCPStressUpdate::numrows(std::string filename){
    int numlines{0};
...
int GRCPStressUpdate::numcols(std::string filename){
    int columncount{0};
    std::ifstream file(filename);
    if (!file.is_open()){
...
void GRCPStressUpdate::readfile(std::string filename){
