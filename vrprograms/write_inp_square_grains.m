function write_inp_square_grains()
...
% The .inp file can be read by MOOSE/rho-CP
    ...
    n_x_grain = 16; % no. of grains along x-direction
    n_y_grain = 16; % no. of grains along y-direction
    n_z_grain = 1; % no. of grains along z-direction
    grain_size = 0.1; % (mm)
    grn_xdir = 0.1; % (mm)
    grn_ydir = 0.1; % (mm)
    grn_zdir = 0.1; % (mm)
    ...
    n_z_ele = 1; % no. of elements along z-direction in each grain
    ...
    for iz = 1:n_node_z
        for iy = 1:n_node_y
            for ix = 1:n_node_x
                ...
                fprintf(file,'%d%s%8.5f%s%8.5f\n',nctr,',',(ix - 1)*el_size,',',(iy - 1)*el_size);
            end
        end
    end
